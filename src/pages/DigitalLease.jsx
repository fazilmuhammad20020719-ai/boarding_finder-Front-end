...
const DigitalLease = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] font-sans antialiased text-[#0f172a] flex flex-col">
      ...
      {/* Top Banner */}
      <div className="bg-[#1e3a8a] text-slate-800 py-4 px-6 md:px-8 shadow-md relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold tracking-tight">Lease Agreement Signing</h1>
          <p className="text-slate-800/70 text-sm mt-0.5">Sunset Apartment - Unit A (12-Month Lease)</p>
        </div>
        
        {/* Progress Tracker */}
        <div className="flex items-center gap-2 text-sm font-bold">
          <div className={`flex items-center gap-1 ${step >= 1 ? 'text-slate-800' : 'text-slate-800/40'}`}>
            <span className="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center text-xs">1</span>
            Review
          </div>
          <div className="w-4 h-0.5 bg-white/20"></div>
          <div className={`flex items-center gap-1 ${step >= 2 ? 'text-slate-800' : 'text-slate-800/40'}`}>
            <span className="w-5 h-5 rounded-full bg-white/20 flex items-center justify-center text-xs">2</span>
            Sign
          </div>
          <div className="w-4 h-0.5 bg-white/20"></div>
          <div className={`flex items-center gap-1 ${step >= 3 ? 'text-[#10b981]' : 'text-slate-800/40'}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${step >= 3 ? 'bg-[#10b981] text-slate-800' : 'bg-white/20'}`}>
              {step >= 3 ? '✓' : '3'}
            </span>
            Complete
          </div>
              ...
              {step === 1 ? (
                <>
                  ...
                  <div className="mt-auto">
                    <button 
                      onClick={() => setStep(2)}
                      className="w-full py-4 bg-[#1952c4] hover:bg-[#1546a8] text-slate-800 font-bold rounded-xl shadow-sm transition-all text-sm"
                    >
                ...
                <form onSubmit={handleSign} className="flex flex-col h-full">
                  ...
                  <div className="mt-auto pt-6 flex gap-3">
                    ...
                    <button 
                      ...
                      className="w-2/3 py-4 bg-[#10b981] hover:bg-[#059669] disabled:bg-[#a7f3d0] disabled:cursor-not-allowed text-slate-800 font-bold rounded-xl shadow-sm transition-all text-sm flex items-center justify-center gap-2"
                    >
                      {isSubmitting ? (
                        <>
          ...
          <div className="w-full max-w-3xl mx-auto bg-white rounded-[24px] shadow-sm border border-[#e2e8f0]/60 p-12 flex flex-col items-center text-center mt-8">
             ...
             <div className="flex gap-4 w-full max-w-md">
                ...
                <Link to="/profile" className="flex-1">
                  <button className="w-full py-4 bg-[#1952c4] hover:bg-[#1546a8] text-slate-800 font-bold rounded-xl shadow-sm transition-colors text-sm">
                    Return to Dashboard
                  </button>
                </Link>
             </div>
