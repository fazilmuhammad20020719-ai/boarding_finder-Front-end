...
const NeighborhoodDetails = () => {
  ...
  const getCategoryColor = (category) => {
    switch(category) {
      case 'Education': return 'text-slate-900 bg-slate-100';
      ...
      <main className="flex-grow flex flex-col md:flex-row relative">
        ...
        <div className="w-full md:w-2/3 h-[50vh] md:h-auto relative bg-[#e5e3df]">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 flex flex-col items-center">
             <div className="bg-[#1952c4] text-slate-800 px-3 py-1.5 rounded-lg shadow-lg font-bold text-sm mb-1 whitespace-nowrap">
               Sunset Apartment
             </div>
          ...
          <div className="absolute right-4 bottom-4 flex flex-col gap-2">
            <button className="w-10 h-10 bg-white/70 backdrop-blur-lg border border-white/80 shadow-[0_8px_30px_rgba(250,204,21,0.15)] hover:shadow-[0_8px_30px_rgba(250,204,21,0.25)] transition-all rounded-xl flex items-center justify-center text-[#0f172a] hover:bg-slate-50 transition-colors">
            ...
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${
                  activeCategory === cat 
                    ? 'bg-[#0f172a] text-slate-800 shadow-sm' 
